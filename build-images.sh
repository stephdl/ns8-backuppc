...
# Reuse existing nodebuilder-backuppc container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-backuppc; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-backuppc -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/tiredofit/backuppc:6.0.2" \
    "${container}"
# Commit the image
